...
polynomial_trend_forecaster_param = {
    'degree': range(1, 11, 1)
}
...
def shrink(x):
    sm = 0
    mn = 100
    l = len(polynomial_trend_forecaster_param[x])
    w = np.zeros(l)
    with lock:
...
def train_model(prm_id, y_train, fh):
    polynomial_trend_forecaster_model = PolynomialTrendForecaster(
        degree=int(polynomial_trend_forecaster_param[0][prm_id[0]]))
    polynomial_trend_forecaster_model.fit(pd.DataFrame(y_train))
    fh_ = ForecastingHorizon(range(1, fh + 1), is_relative=True)
    y_predict = polynomial_trend_forecaster_model.predict(fh_)
...
def best_param(y_train, y_test, fh):
    ...
    with lock:
        ...
        for i in range(n):
            k = 0
            mx = 0
            for j in range(len(polynomial_trend_forecaster_param[i])):
                if ar[i][j][0] > mx:
...
def shrink_polynomial_trend_forecaster(y_train, y_test, fh, rd, njob):
