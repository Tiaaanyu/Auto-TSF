...
def shrink(x):
    sm = 0
    mn = 100
    l = len(bagging_forecaster_param[x])
    w = np.zeros(l)
    with lock:
...
def train_model(prm_id, y_train, fh):
    bagging_forecaster_model = BaggingForecaster(STLBootstrapTransformer(sp=int(bagging_forecaster_param[0][prm_id[0]])),
                                                 NaiveForecaster(sp=int(bagging_forecaster_param[0][prm_id[0]])))
    bagging_forecaster_model.fit(pd.DataFrame(y_train))
    fh_ = ForecastingHorizon(range(1, fh + 1), is_relative=True)
    y_predict = bagging_forecaster_model.predict(fh_)
...
def best_param(y_train, y_test, fh):
    ...
    with lock:
        ...
        for i in range(n):
            k = 0
            mx = 0
            for j in range(len(bagging_forecaster_param[i])):
                if ar[i][j][0] > mx:
...
def shrink_bagging_forecaster(y_train, y_test, fh, rd, njob):
