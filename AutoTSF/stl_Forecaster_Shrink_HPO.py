...
stl_forecaster_param = {
    'sp': range(2, 100, 1),
    'seasonal': range(7, 100, 1)
}
...
def train_model(prm_id, y_train, fh):
    stl_forecaster_model = STLForecaster(sp=int(stl_forecaster_param[0][prm_id][0]),
                                         seasonal=int(stl_forecaster_param[1][prm_id][1]))
    stl_forecaster_model.fit(pd.DataFrame(y_train))
    fh_ = ForecastingHorizon(range(1, fh + 1), is_relative=True)
    y_predict = stl_forecaster_model.predict(fh_)
...
def best_param(y_train, y_test, fh):
    ...
    with lock:
        ...
        for i in range(n):
            k = 0
            mx = 0
            for j in range(len(stl_forecaster_param[i])):
                if ar[i][j][0] > mx:
...
def shrink_stl_forecaster(y_train, y_test, fh, rd, njob):
