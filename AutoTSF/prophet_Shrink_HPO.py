...
def train_model(prm_id, y_train, fh):
    prophet_model = Prophet(seasonality_mode=prophet_param[0][prm_id[0]])
    prophet_model.fit(pd.DataFrame(y_train))
    fh_ = ForecastingHorizon(range(1, fh + 1), is_relative=True)
    y_predict = prophet_model.predict(fh_)
...
def best_param(y_train, y_test, fh):
    ...
    with lock:
        ...
        for i in range(n):
            k = 0
            mx = 0
            for j in range(len(prophet_param[i])):
                if ar[i][j][0] > mx:
...
def shrink_prophet(y_train, y_test, fh, rd, njob):
